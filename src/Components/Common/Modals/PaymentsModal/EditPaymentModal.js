...
import { getPaymentTypeAPI, putPaymentTypeAPI } from "../../../../Utils/API/Payments";
import {
  Row,
  Col,
  Modal,
...
const EditPaymentModal = ({ editPaymentModal, setEditPaymentModal, onClickEditPayment, paymentID }) => {

  //data request
  const [dataPayment, setDataPayment] = useState({})
  useEffect(() => {
    getPaymentTypeAPI(paymentID).then((resp) =>{
      setDataPayment(resp.data.data)
    })
  }, [paymentID]);
  ...
  const validationType = useFormik({
    ...
    initialValues: {
      ...
      default_label: dataPayment.default_label
    },
    ...
    onSubmit: (values) => {
      ...
      putPaymentTypeAPI(paymentID, data)
        .then((resp) => {
          if (resp.data.status === 200) {
            Swal.fire("Edited!", "Payment Type has been edited.", "success").then(
              () => {
  ...
  return (
    ...
    >
      ...
      >
        <h1 className="modal-title mt-0 text-white">+ Edit Payment Type</h1>
        <button
          onClick={() => {
            setEditPaymentModal(false);
          }}
