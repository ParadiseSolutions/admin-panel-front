...
const EditWebsiteModal = ({
    ...
    useEffect(() => {
      if (data && dataAreas) {
        ...
        dataAreas.forEach((element) => {
          if (data.service_areas_ids.includes(element.id)) {
  ...
  const validationType = useFormik({
    ...
    initialValues: {
      ...
      url: data ? data.url : "",
      cpanel_account: data ? data.cpanel_account : "",
      root_folder: data ? data.root_folder : "",
      user_folder: data ? data.user_folder : "",
    ...
    onSubmit: (values) => {
      let dataBackend = {
        ...
        service_area_ids:
          selectionID.length > 0 ? selectionID : data.service_areas_ids,
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h1 className="modal-title mt-0 text-white">+ Edit Website</h1>
          <button
            onClick={() => {
              setEditModal(false);
            }}
