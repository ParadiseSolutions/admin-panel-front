...
const AddNewPrivateTour = ({
  ...
  const validationType = useFormik({
    ...
    initialValues: {
      min: dataEdit ? dataEdit.pricedetails?.filter((x) => x.pricing_option_id === 7)[0]?.min : "",
      max: dataEdit ? dataEdit.pricedetails?.filter((x) => x.pricing_option_id === 7)[0]?.max : "",
  ...
  useEffect(() => {
    if (dataEdit && addNewPrivateTour && priceCollect) {
      ...
      setPriceCollectSelected(dataEdit.pricedetails.filter((x) => x.pricing_option_id === 9)[0]
        ?.source_id)
      let priceCollectSe = priceCollect.filter(x => x.id === dataEdit.pricedetails.filter((x) => x.pricing_option_id === 9)[0]
        ?.source_id)
